...
export default function NoticeTableUI({ items }: { items: NoticeUI[] }) {
  ...
  const closeViewModal = () => { setViewModalOpen(false); setNotice(null); setViewError(null); setLoadingView(false); };
  ...
  const closeStatusModal = () => { setStatusModalOpen(false); setStatusTargetId(null); setStatusCurrent(null); };
      ...
      <div className="overflow-x-auto rounded-lg border border-gray-200 dark:border-gray-700">
        <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
          ...
          <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-100 dark:divide-gray-700">
            {items.length === 0 ? (
              <tr><td colSpan={7} className="px-4 py-6 text-center text-sm text-gray-500">No notices found</td></tr>
            ) : (
              items.map((n) => (
                <tr key={n.id} className="hover:bg-grwhen i open to edit any notice to update status the current status should be selected already in the statusModelay-50 dark:hover:bg-gray-700 transition">
                  ...
                  <td className="px-4 py-4 text-sm text-gray-600 dark:text-gray-300">{new Date(n.publishedOn).toLocaleDateString("en-GB", { day: "2-digit", month: "short", year: "numeric" })}</td>
                  <td className="px-4 py-4"><StatusBadge status={n.status} /></td>
                  <td className="px-4 py-4">
                    <div className="flex justify-end items-center gap-3">
                      <button onClick={() => openModalWith(n.id)} className="text-gray-500 hover:text-indigo-600" aria-label={`View notice ${n.title}`}><EyeIcon className="h-5 w-5" /></button>

                      <button onClick={() => openStatusModal( n.id, n.status === "Unpublished" ? "Draft" : (n.status as "Published" | "Draft" | "Unpublished") ) } className="text-gray-500 hover:text-indigo-600" aria-label={`Update status for ${n.title}`}>
