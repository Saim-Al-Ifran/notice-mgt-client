...
export default function NoticeTableUI({ items }: { items: NoticeUI[] }) {
  ...
  return (
    <>
            ...
            {items.length === 0 ? (
              ...
              items.map((n) => (
                ...
                >
                  ...
                  <td className="px-4 py-4 text-sm text-gray-600 dark:text-gray-300">
                    {new Date(n.publishedOn).toLocaleDateString("en-GB", {
                      day: "2-digit",
                      month: "short",
                      year: "numeric",
                    })}
                  ...
                  <td className="px-4 py-4">
                    <div className="flex justify-end items-center gap-3">
                      <button
                        onClick={() => openModalWith(n.id)}
                        className="text-gray-500 hover:text-indigo-600"
                        aria-label={`View notice ${n.title}`}
                      >
      ...
      <NoticeModal open={modalOpen} onClose={closeModal} loading={loading} error={error} notice={notice} />
    </>
  );
}
